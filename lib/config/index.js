...
async function getConfig() {
  const env = envSchema({
    ...
    schema: S.object()
      ...
      .prop('ENFORCE_ONSET_DATE_RANGE_ERROR', S.boolean())
      ...
      .prop('ENABLE_DCC', S.boolean())
      .prop('ENX_LOGO_SUPPORTED', S.boolean())
  })
  ...
  const config = {
    ...
    routes: {
      dcc: /true/i.test(env.ENABLE_DCC),
      enx: /true/i.test(env.ENX_LOGO_SUPPORTED)
    },
    pgPlugin: {
    ...
    sms: {
      ...
      enforceSymptomDateRangeError: env.ENFORCE_ONSET_DATE_RANGE_ERROR,
  ...
  if (isProduction) {
    ...
    config.cors.credentials = /true/i.test(
      await getOptionalParameter('push_cors_credentials', false)
    )

    config.routes.dcc = await getOptionalParameter('enable_dcc', false)
    config.routes.enx = await getOptionalParameter('enx_logo_supported', false)
    ...
    config.sms.enforceSymptomDateRangeError = /true/i.test(
      await getOptionalParameter('enforce_onset_date_range_error', false)
    )
    ...
    config.sms.deeplinkAllowed = /true/i.test(
      await getOptionalParameter('security_code_deeplinks_allowed', false)
    )
    config.issueProxy.url = await getOptionalParameter('issue_proxy_url', '')
