...
const metricsDCC = {
  description: 'Retrieve dcc stats between given dates',
  query: S.object()
    .dependencies({
      period: S.not(S.required(['startDate', 'endDate']))
    })
  ...
  response: {
    200: S.array().items(
      S.object()
        .prop('eventDate', S.string().format('date-time'))
        .prop('location', S.string().required())
        .prop('certType', S.string().required())
        .prop('scanPassed', S.string().required())
        .prop('failureReason', S.string())
        .prop('issuingCountry', S.string().required())
    )
  }
}

module.exports = { list, payloads, metricsENX, metricsDCC }
