...
async function metrics(server, options, done) {
  ...
  if (options.routes.enx) {
    console.log('ENX Stats enabled')
  ...
  if (options.routes.dcc) {
    console.log('DCC Stats enabled')

    server.route({
      method: 'GET',
      url: '/metrics/dcc',
      schema: schema.metricsDCC,
      handler: async (request, response) => {
        // await request.authenticate(['metrics'])
        const { startDate, endDate, period, limit, offset } = request.query
        const { accept } = request.headers
