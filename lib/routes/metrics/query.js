...
const enxOnboarding = ({ startDate, endDate, limit, offset }) => {
  const query = SQL`
    SELECT event_date AS "date", all_counts as all, success_counts as "success", settings_counts as settings, enbuddy_counts as enbuddy, healthenbuddy_counts as healthenbuddy
    FROM enx_onboarding_requests
    WHERE event_date >= ${new Date(startDate)}
    AND event_date <= ${new Date(endDate)}
...
const dccStats = ({ startDate, endDate, limit, offset }) => {
  const query = SQL`
    SELECT event_date AS "eventDate", app_location as location, cert_type as "certType", scan_passed as "scanPassed", COALESCE(failure_reason, '') as "failureReason", issuing_country as "issuingCountry"
    FROM dcc_stats
    WHERE event_date >= ${new Date(startDate)}
    AND event_date <= ${new Date(endDate)}
    ORDER BY event_date ASC
